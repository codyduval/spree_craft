class Admin::OrdersController < Admin::BaseController
  ...
  def index
    ...
    @orders = Order.includes([:user, :shipments, :payments]).order('created_at DESC').search(params[:search]).result(:distinct => true).page(params[:page]).per(Spree::Config[:orders_per_page])
  end
